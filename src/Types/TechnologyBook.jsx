...
const TechnologyBook = () => {
  ...
  const filterBooks = () => {
    ...
    const businessBooks = allBooks
      .filter((item) => item.categoryid === businessCategory.id)
      .map((item, index) => ({
        key: index,
        bookID: item.id,
        title: item.title,
        ...
        categoryID: "Techonology", // Example transformation
        publishedYear: item.published_year,
        quantity: item.quantity,
        price:"$ "+item.price,
      }));

    setData(businessBooks);
  };
  ...
  return (
    <div>
      <h1 className="w-full mb-10 text-3xl">Business Books List</h1>
      <Table
        columns={columns}
        dataSource={data}
